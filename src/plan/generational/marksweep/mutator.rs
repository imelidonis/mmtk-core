...
pub fn genmarksweep_mutator_release<VM: VMBinding>(mutator: &mut Mutator<VM>, _tls: VMWorkerThread) {
...
pub fn create_genmarksweep_mutator<VM: VMBinding>(
    mutator_tls: VMMutatorThread,
    mmtk: &'static MMTK<VM>,
) -> Mutator<VM> {
    let genmarksweep = mmtk.get_plan().downcast_ref::<GenMarkSweep<VM>>().unwrap();
    let config = MutatorConfig {
        allocator_mapping: &ALLOCATOR_MAPPING,
        space_mapping: Box::new(create_gen_space_mapping(
            mmtk.get_plan(),
            &genmarksweep.gen.nursery,
        )),
        prepare_func: &unreachable_prepare_func,
        release_func: &genmarksweep_mutator_release,
    };
